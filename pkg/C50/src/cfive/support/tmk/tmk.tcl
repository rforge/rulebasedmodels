...
proc module {modules} {
	...
	if {[module_loaded $mod]} {
	    set actVers [set ::${mod}::VERSION]
	    if {$actVers eq $modver} {
...
proc module_loaded {mod} {
    return [lcontains $::MODULES ${mod}]
}
...
proc __eval_module_macros {names} {
	...
	foreach var [array names MODULE_ACTION] {
		...
		uplevel \#0 namespace eval ::$name \
		    [list $MODULE_ACTION($var)]
	    }
	}
    }
    return $found
...
proc arraykeyvalexistp {_arrayName _key _val} {
    ...
    if {[arraykeyexistp array $_key]} {
        foreach elem $array($_key) {
            if {$elem == $_val} {
    ...
    #if he key doesn't exist in the list, and value is allowed
    return 1
}
...
# \param key "key=value", ex. "addressmode=64bit"
# \return true if key is exist and set is succeeded.
#
proc ::tmk::set_runmodestat {key val} {
    global __AvailableRunModeStatList
    global __RunModeStatList

    if {[arraykeyvalexistp __AvailableRunModeStatList $key $val]} {
	set __RunModeStatList($key) $val
	return 1
    } else {
	dbg_msg "could not find mode = $key, value = $val, no set."
	return 0
    }
}
